...
![image (1).png](F:\笔记\mysql\images\mysql-work-level.png)
...
<img src="https://s0.lgstatic.com/i/image6/M01/42/07/CioPOWCvUQuALARKAASaAs1r1GE632.png" alt="image (2).png" style="zoom:50%;" />
...
字段 o_orderstatus 上创建索引虽然能解决问题，但是成本非常大的，需要维护索引idx_orderstatus 的各种变更，性能会受到一定影响。有什么其他更好的方法来判断还处在支付过程中的订单吗？

答案：通过订单的时间字段来进行查询，查看订单状态，这样可以比米娜在状态列上创建索引。

比如，查询五分钟的订单状态，超过15分钟的订单就将状态设置为关闭。