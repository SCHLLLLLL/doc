...
但由于上述表结构的索引设计时，索引 ORDERS_FK1 仅对列 O_CUSTKEY 排序，因此在取出用户 147601 的数据后，**还需要一次额外的排序**才能得到结果，可通过命令EXPLAIN验证：
...
## 思考题
组合索引可以避免 WHERE a = ? ORDER BY b 这条的额外排序问题，在现有的业务中，我们如何发现可以的 SQL，通过组合索引进行优化呢？

下面这条 SQL 该如何创建组合索引？同时对比 MySQL 5.7 和 MySQL 8.0 版本，查看优化器的执行计划，看看 8.0 在 5.7 版本基础上，还有哪些进一步的优化：
...
1. 用慢查询日志其实比较难发现问题，因为有a索引不会特别慢，不一定能进入慢查询日志，可以通过下面的sql语句查看：

   ```sql
   SELECT query,rows_sorted FROM x$statement_analysis WHERE db NOT IN('sys','mysql') AND rows_sorted != 0;
   ```

2. MySQL8.0 支持不同方向的索引排序，MySQL5.7 仅支持一个方向的。因此8.0可以创建(o_orderdate desc, o_orderstatus asc)的索引。而5.7 即使创建不同排序方向的索引，也需要额外排序。